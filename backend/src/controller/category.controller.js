...
const getParticularCategory = asyncHandler(async (req, res) => {
  ...
  if (!mongoose.isValidObjectId(categoryId)) {
    throw new ApiError(404, "Category not found, invalid Category id");
  }

  const findCategory = await Category.findById(categoryId);

  if (!findCategory) {
    throw new ApiError(404, "not found any category");
  }

  return res
    .status(200)
    .json(new ApiResponse(200, findCategory, "Here is Your Category"));
});
...
const deleteCategory = asyncHandler(async (req, res) => {
  ...
  if (!categoryId) {
    throw new ApiError(400, "Category id is required to delete");
  }
